...
data class SecurityProperties(
    ...
    val auth: SecurityAuthProperties = SecurityAuthProperties(),
    /** CORS設定情報  */
    val cors: SecurityCorsProperties = SecurityCorsProperties()
)
...
data class SecurityAuthProperties(
    ...
    /** 認証対象パス  */
    var path: Array<String> = arrayOf("/api/**"),
    /** 認証対象パス(管理者向け)  */
    var pathAdmin: Array<String> = arrayOf("/api/admin/**"),
    /** 認証除外パス(認証対象からの除外)  */
    var excludesPath: Array<String> = arrayOf("/api/system/job/**"),
    /** 認証無視パス(フィルタ未適用の認証未考慮、静的リソース等)  */
    var ignorePath: Array<String> = arrayOf("/css/**", "/js/**", "/img/**", "/**/favicon.ico"),
    /** ログインAPIパス  */
